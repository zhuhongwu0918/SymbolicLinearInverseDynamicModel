...
syms q1 m1 L1 Lc1 real
syms Ixx1 Ixy1 Ixz1 Iyy1 Iyz1 Izz1  real
syms cx1  cy1 cz1 real

syms q2 m2 L2 Lc2 real
syms Ixx2 Ixy2 Ixz2 Iyy2 Iyz2 Izz2  real
syms cx2  cy2 cz2 real
...
mass_centerOi= [Lc1,0,0;Lc2,0,0];
          
mass1 = [m1];
mass2 = [m2];
inertia_C1 = [Ixx1 Ixy1 Ixz1;
             Ixy1 Iyy1 Iyz1;
             Ixz1 Iyz1 Izz1];%inertia tensor at joint 1 origin coordinate.ie. IOi
         %IOi = ICi+m1*crossMatrix(r1)*crossMatrix(r1)'; r1=[Lc1;0;0]; 
inertia_C2 = [Ixx2 Ixy2 Ixz2;
             Ixy2 Iyy2 Iyz2;
             Ixz2 Iyz2 Izz2];
inertia_tensor_Clist(:,:,1) = inertia_C1;
inertia_tensor_Clist(:,:,2) = inertia_C2;
...
%         dvOi = [0,0 -g]';
    ...
    w(:,:,i+1)   = R(:,:,i+1)*wi + dq(i+1)*z(:,i+1);%公式1
    dw(:,:,i+1)  = R(:,:,i+1)*dwi + cross(R(:,:,i+1)*wi,dq(i+1)*z(:,i+1)) + ddq(i+1)*z(:,i+1);
...
g_vec = [g,0 0]';% 线加速度，以基坐标为准，第一个关节会体现在旋转矩阵的dh参数的alpha上，基坐标x向下，则为g00。第一根杆是上下摆动，而不是平面摆动。
%          g_vec = [0 0 -g]';%第一根杆是平面摆动。
Phi_1 = [m1, m1*cx1,m1*cy1,m1*cz1,Ixx1,Ixy1,Ixz1,Iyy1,Iyz1,Izz1]';
Phi_2 = [m2, m2*cx2,m2*cy2,m2*cz2,Ixx2,Ixy2,Ixz2,Iyy2,Iyz2,Izz2]';

f_1_1 = [dvO(:,:,1)-g_vec , crossMatrix(dw(:,:,1)) + crossMatrix(w(:,:,1))*crossMatrix(w(:,:,1)),zeros(3,6)]*[m1, m1*cx1,m1*cy1,m1*cz1,Ixx1,Ixy1,Ixz1,Iyy1,Iyz1,Izz1]';
n_1_1 = [zeros(3,1), crossMatrix((g_vec-dvO(:,:,1))), kMatrix(dw(:,:,1))+crossMatrix(w(:,:,1))*kMatrix(w(:,:,1))]*[m1, m1*cx1,m1*cy1,m1*cz1,Ixx1,Ixy1,Ixz1,Iyy1,Iyz1,Izz1]';
A_1 = [dvO(:,:,1)-g_vec , crossMatrix(dw(:,:,1)) + crossMatrix(w(:,:,1))*crossMatrix(w(:,:,1)),zeros(3,6);zeros(3,1), crossMatrix((g_vec-dvO(:,:,1))), kMatrix(dw(:,:,1))+crossMatrix(w(:,:,1))*kMatrix(w(:,:,1))];
%w_1_1 = A_1*Phi_1
w_1_1 = [f_1_1;n_1_1];
isequal(A_1*Phi_1,w_1_1)
T1 = [R(:,:,1)',zeros(3,3);crossMatrix(P(:,:,1))*R(:,:,1)',R(:,:,1)'];

f_2_2 =[dvO(:,:,2)-g_vec , crossMatrix(dw(:,:,2)) + crossMatrix(w(:,:,2))*crossMatrix(w(:,:,2)),zeros(3,6)]*[m2, m2*cx2,m2*cy2,m2*cz2,Ixx2,Ixy2,Ixz2,Iyy2,Iyz2,Izz2]';
% f_2_2 =[dvO(:,:,2) , crossMatrix(dw(:,:,2)) + crossMatrix(w(:,:,2))*crossMatrix(w(:,:,2)),zeros(3,6)]*[m2, m2*cx2,m2*cy2,m2*cz2,Ixx2,Ixy2,Ixz2,Iyy2,Iyz2,Izz2]';
n_2_2 = [zeros(3,1), crossMatrix((g_vec-dvO(:,:,2))), kMatrix(dw(:,:,2))+crossMatrix(w(:,:,2))*kMatrix(w(:,:,2))]*[m2, m2*cx2,m2*cy2,m2*cz2,Ixx2,Ixy2,Ixz2,Iyy2,Iyz2,Izz2]';
A_2 = [dvO(:,:,2), crossMatrix(dw(:,:,2)) + crossMatrix(w(:,:,2))*crossMatrix(w(:,:,2)),zeros(3,6);zeros(3,1), crossMatrix((g_vec-dvO(:,:,2))), kMatrix(dw(:,:,2))+crossMatrix(w(:,:,2))*kMatrix(w(:,:,2))];

%w_2_2 = A_2*Phi_2
w_2_2 = [f_2_2;n_2_2];
isequal(A_2*Phi_2,w_2_2)
...
tau_1 = [0,0,0,0,0,1]*w_1;
tau_2 = [0,0,0,0,0,1]*w_2;
...
z = [0,0,0,0,0,1];
K_tau = [z*U_1_1, z*U_1_2;z*zeros(6,10),z*U_2_2]
