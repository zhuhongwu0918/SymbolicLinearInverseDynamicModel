...
%           theta      d        a        alpha 
L(1)=Link([theta1      d1        a1        alpha1]); L(1).offset=0.5*pi;L(1).qlim=[-pi,pi];
L(2)=Link([theta2      d2        a2        alpha2]); L(2).offset=0.5*pi;L(2).qlim=[-pi,pi];
% 移动关节需要特别指定关节类型--jointtype
L(3)=Link([theta3      d3        a3        alpha3]);  L(3).qlim=[500,1000]; L(3).jointtype='P';
% 把上述连杆“串起来”
Scara=SerialLink(L,'name','Threelink');
% 定义机器人基坐标和工具坐标的变换
Scara.base = transl(0 ,0 ,0);
Scara.base = trotx(0.5*pi);
